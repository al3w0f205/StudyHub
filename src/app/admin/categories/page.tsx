...
export const metadata: Metadata = { title: "Gestión de Categorías" };
...
export default async function CategoriesPage({ searchParams }: PageProps) {
  ...
  let careers: any[] = [], categories: any[] = [], selectedCareer: any = null;
  let hasError = false;
  try {
    careers = await prisma.career.findMany({ orderBy: { name: "asc" } });
    ...
    selectedCareer = careers.find((c) => c.id === careerId);
  } catch (error) {
    ...
    <div>
      ...
      >
        <Link
          ...
          className={`btn btn-sm ${!careerId ? "btn-primary" : "btn-secondary"}`}
        >
          Todas
        </Link>
        {careers.map((c) => (
          <Link
            ...
            className={`btn btn-sm ${
              careerId === c.id ? "btn-primary" : "btn-secondary"
            }`}
          >
            {c.icon || "📚"} {c.name}
          </Link>
      ...
      {careers.length > 0 && (
        ...
        >
          ...
          <form action={createCategory}>
            ...
            >
              <div>
                <label htmlFor="cat-career" className="label">
                  Carrera
                </label>
                <select
                  id="cat-career"
            ...
            <div>
              <label htmlFor="cat-theory" className="label">
                Contenido de Teoría (Markdown/LaTeX)
              </label>
              <textarea
                id="cat-theory"
                name="theory"
      ...
      {categories.length === 0 ? (
        <div className="solid-card empty-state">
          ...
          <p>
            No hay categorías{selectedCareer ? ` en ${selectedCareer.name}` : ""}
          </p>
        </div>
      ) : (
                ...
                <tr key={cat.id}>
                  ...
                  <td>
                    <div style={{ display: "flex", gap: "0.5rem" }}>
                      <Link
                        href={`/admin/questions?categoryId=${cat.id}`}
