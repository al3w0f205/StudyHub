...
export const metadata: Metadata = { title: "Gestión de Preguntas" };
...
async function createQuestion(formData: FormData) {
  ...
  const correctIndexInput = formData.get("correctIndex");
  const correctIndex = correctIndexInput ? parseInt(correctIndexInput as string, 10) : NaN;
  const hint = formData.get("hint") as string | null;
  const explanation = formData.get("explanation") as string | null;
  
  const options: string[] = [];
  for (let i = 0; i < 4; i++) {
    const opt = formData.get(`option-${i}`) as string | null;
    if (opt && opt.trim()) options.push(opt.trim());
  }
  
  if (options.length < 2 || isNaN(correctIndex) || correctIndex < 0 || correctIndex >= options.length) return;
...
export default async function QuestionsPage({ searchParams }: PageProps) {
  ...
  let categories: any[] = [], questions: any[] = [], total: number = 0, totalPages: number = 0;
  let hasError = false;
  try {
    ...
    const [qs, count] = await Promise.all([
    ...
    <div>
      ...
      {categories.length > 0 && (
        <details className="solid-card" style={{ marginBottom: "1.5rem" }}>
          <summary
            style={{ padding: "1rem 1.5rem", cursor: "pointer", fontWeight: "700" }}
          >
            ➕ Nueva Pregunta
          </summary>
          <form action={createQuestion} style={{ padding: "0 1.5rem 1.5rem" }}>
            <div style={{ display: "grid", gap: "1rem" }}>
              ...
              >
                <div>
                  <label className="label">Correcta (0-3)</label>
                  <input
                    name="correctIndex"
                    type="number"
                    min="0"
