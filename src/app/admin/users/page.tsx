...
async function updateAllowedCareers(formData: FormData) {
  "use server";
  await requireAdmin();
  const id = formData.get("id") as string;
  const careerSlug = formData.get("careerSlug") as string;
  ...
  let allowed = user.allowedCareers
    ? user.allowedCareers.split(",").filter((c) => c.trim())
    : [];
  ...
  if (careerSlug === "general" && action === "add") {
    ...
    action === "add" &&
    validCareerSlugs.has(careerSlug) &&
    !allowed.includes(careerSlug)
  ) {
...
export default async function UsersPage() {
  ...
  let users: any[] = [], careers: any[] = [];
  let hasError = false;
  try {
    [users, careers] = await Promise.all([
                        ...
                        >
                          ...
                          <form action={updateAllowedCareers}>
                            ...
                            <input
                              ...
                              name="action"
                              value={
                                allowedList.length === careers.length
                                  ? "remove"
                                  : "add"
                              }
                            />
