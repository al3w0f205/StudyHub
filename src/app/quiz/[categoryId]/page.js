...
export default async function QuizPage({ params }) {
  ...
  try {
    ...
    const userResponses = session?.user?.id ? await prisma.questionResponse.findMany({
      where: { 
    ...
    const quizSeed = hashSeed(session?.user?.id || "anonymous");
    ...
    const shuffled = seededShuffle(pendingQuestions, quizSeed).map((question) => {
      ...
      const watermarkedText = encodeForensic(question.text, session?.user?.id || "anonymous");
      const watermarkedExplanation = question.explanation 
        ? encodeForensic(question.explanation, session?.user?.id || "anonymous") 
        : null;
    ...
    return (
      <div style={{ padding: "4rem 2rem", textAlign: "center" }}>
        <h1 style={{ fontSize: "2rem", marginBottom: "1rem" }}>⚠️ Error de Conexión</h1>
        <p style={{ color: "var(--text-tertiary)", maxWidth: "500px", margin: "0 auto" }}>
          No pudimos conectar con la base de datos. Por favor, verifica que el servidor de base de datos esté funcionando e inténtalo de nuevo.
        </p>
      </div>
    );
  }
