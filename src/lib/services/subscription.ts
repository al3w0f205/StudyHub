 ...
 *
 * @param {any} user - Objeto del usuario (de session o DB), debe contener `id` y `role`
 ...
 */
export async function hasCareerAccess(
  ...
  const isSubActive =
    ...
    new Date(dbUser.subscriptionExpiry) > new Date();
  ...
  const allowedCareers =
    dbUser.allowedCareers?.split(",").filter(Boolean) ?? [];
