...
export const metadata: Metadata = { title: "Gestión de Pagos" };
...
async function approvePayment(formData: FormData) {
  ...
  const userUpdateData: any = { subscriptionExpiry: expiry };
  ...
  if (requestedCareers) {
    ...
    let allowed = user?.allowedCareers
    ...
    const requested = requestedCareers
      .split(",")
      .filter((slug) => validCareerSlugs.has(slug));
...
async function rejectPayment(formData: FormData) {
  ...
  await requireAdmin();
  const id = formData.get("id") as string;
  const notes = (formData.get("notes") as string) || "Rechazado";
  await prisma.paymentRequest.update({
    where: { id },
    data: { status: "REJECTED", adminNotes: notes, reviewedAt: new Date() },
  });
  redirect("/admin/payments");
}
...
export default async function PaymentsPage() {
  ...
  return (
    <div>
      ...
      {pending.length === 0 ? (
        <div
          className="solid-card empty-state"
                ...
                {reviewed.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td style={{ fontSize: "0.8125rem" }}>
                      {p.reviewedAt
                        ? new Date(p.reviewedAt).toLocaleDateString("es-ES")
                        : "—"}
                    </td>
