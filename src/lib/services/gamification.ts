 ...
 * @returns {Promise<any>} Usuario actualizado o null si no existe
 */
export async function updateUserStats(userId: string, pointsToAdd: number) {
 ...
 * @returns {Promise<Array<any>>} Array de badges recién desbloqueados (para mostrar toast)
 */
export async function awardBadges(userId: string, calculatedScore: number) {
  ...
  const [user, allBadges, existingUserBadges] = await Promise.all([
    prisma.user.findUnique({
      where: { id: userId },
      select: { streak: true, totalPoints: true },
    }),
  ...
  if (user.streak >= 3 && !earnedSlugs.includes("streak_3"))
    badgesToAward.push("streak_3");
  if (user.streak >= 7 && !earnedSlugs.includes("streak_7"))
    badgesToAward.push("streak_7");
  if (calculatedScore === 100 && !earnedSlugs.includes("score_100"))
    badgesToAward.push("score_100");
  if (user.totalPoints >= 1000 && !earnedSlugs.includes("points_1000"))
    badgesToAward.push("points_1000");
  if (user.totalPoints >= 5000 && !earnedSlugs.includes("points_5000"))
  ...
  if (quizCount >= 10 && !earnedSlugs.includes("quizzes_10"))
    badgesToAward.push("quizzes_10");
  if (quizCount >= 50 && !earnedSlugs.includes("quizzes_50"))
