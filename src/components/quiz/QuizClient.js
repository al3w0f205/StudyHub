// =============================================================================
// StudyHub — QuizClient: Orquestador Principal del Motor de Cuestionarios
// =============================================================================
// Este es el componente central del sistema de quiz. Actúa como "state orchestrator"
// que coordina 7 sub-componentes (en client-parts/) sin contener rendering pesado.
//
// MODOS DE ESTUDIO:
//   - "quiz"       → Cuestionario estándar con feedback inmediato
//   - "flashcards" → Tarjetas flip con pregunta/respuesta
//   - "theory"     → Vista de material de estudio (Markdown/LaTeX)
//   - Examen (sub-modo) → Quiz con presión de tiempo + modo enfoque (Zen)
//
// ESTADO PRINCIPAL:
//   - view          → Modo actual (quiz/flashcards/theory)
//   - current       → Índice de la pregunta actual
//   - selected      → Índice de la respuesta seleccionada (null = no respondida)
//   - score/answered → Contadores de la sesión actual
//   - history       → Registro de respuestas para envío batch
//   - completedCount → Preguntas correctas acumuladas (progreso de categoría)
//
// RESILIENCIA OFFLINE:
//   - Si saveProgress() falla, las respuestas se guardan en localStorage
//     como "studyhub_sync_queue" y se sincronizan cuando vuelve la conexión.
//   - Listener de window "online" para sincronización automática.
//
// ADMIN:
//   - Si isAdmin=true, aparece el botón "Editar" que abre AdminQuestionEditor
//     para edición in-situ de preguntas vía PATCH /api/admin/questions.
// =============================================================================
...
export default function QuizClient({ 
  ...
  // State
  const [view, setView] = useState(questions.length === 0 ? "theory" : "quiz");
  ...
  }, [categoryId, history, isOfflineMode, addToast]);
  ...
  const nextQuestion = async () => {
    ...
    if (current + 1 >= total) {
      ...
      await saveProgress({ type: "final", score: Math.max(0, Math.min(100, finalScore)) });
  ...
  const resetProgress = async () => {
    if (!window.confirm("¿Estás seguro de que quieres reiniciar tu progreso?")) return;
    setIsResetting(true);
    try {
      const res = await fetch(`/api/quiz-progress?categoryId=${categoryId}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const reportError = async (reason) => {
    if (!q || isReporting) return;
    if (String(reason || "").trim().length < 8) return alert("Describe más el error.");
    setIsReporting(true);
    try {
  ...
  const saveOffline = () => {
    const offlineData = { id: categoryId, name: categoryName, career: careerName, questions, theory, savedAt: new Date().toISOString() };
    try {
      const existing = JSON.parse(localStorage.getItem("studyhub_offline") || "[]").filter(item => item.id !== categoryId);
      existing.push(offlineData);
      localStorage.setItem("studyhub_offline", JSON.stringify(existing));
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      for (const item of queue) {
        try {
          const res = await fetch("/api/quiz-progress", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(item) });
          if (res.ok) success++;
        } catch (e) { break; }
      }
      if (success > 0) {
        localStorage.setItem("studyhub_sync_queue", JSON.stringify(queue.slice(success)));
        addToast(`Sincronizadas ${success} respuestas 🔄`, "info");
      }
    };
  ...
  if (finished && total > 0) {
    return <QuizResults score={score} total={total} isExamMode={isExamMode} restart={restart} />;
  }

  return (
    <div className={`quiz-layout ${isZenMode ? 'zen-mode' : ''}`}>
      {sidebarOpen && <div className="quiz-sidebar-overlay" onClick={() => setSidebarOpen(false)} />}

      {!isZenMode && (
        <QuizSidebar 
          {...{
            isZenMode, sidebarOpen, setSidebarOpen, selectedCareerId, setSelectedCareerId,
            careers, filteredCategories, categoryId, view, setView, isExamMode, total,
            startExamMode, restart, theory, isTimePressure, setIsTimePressure,
            setIsZenMode, resetProgress, isResetting
          }}
        />
      )}
      ...
      <div className="quiz-main" style={{ maxWidth: isZenMode || view === "theory" || view === "flashcards" ? 850 : "none", margin: isZenMode || view === "theory" || view === "flashcards" ? "0 auto" : "0", width: "100%" }}>
        {view === "theory" ? (
          <TheoryView {...{ careerName, categoryName, isOfflineMode, saveOffline, total, setView, theory }} />
        ) : view === "flashcards" ? (
          <FlashcardsView {...{ current, total, q, isFlipped, setIsFlipped, setCurrent, nextQuestion }} />
        ) : (
          <>
            <QuizHeader 
              {...{
                careerName, categoryName, isOfflineMode, isExamMode, current, total,
                initialCompletedCount, totalQuestionsInCategory, isTimePressure, selected,
                timeLeft, score, answered, isZenMode, setIsZenMode, setIsExamMode, isAdmin,
                onEdit: () => { setEditedQuestion({...q}); setIsEditing(true); }
              }}
            />
            ...
            {total === 0 ? (
              <div className="solid-card animate-fade-in" style={{ padding: "4rem 2rem", textAlign: "center" }}>
                ...
                <p style={{ color: "var(--text-secondary)", marginBottom: "2rem" }}>Has respondido correctamente todas las preguntas.</p>
              ...
              <QuizQuestion {...{ q, selected, handleAnswer, isExamMode }} />
            )}
            ...
            {total > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem", flexWrap: "wrap", gap: "0.5rem" }}>
                <div className="quiz-actions-row" style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
                  ...
                  <button onClick={() => setReportOpen(!reportOpen)} className="btn btn-ghost" style={{ color: "var(--text-tertiary)", fontSize: "0.8125rem", opacity: 0.8 }}>
                    🚩 Reportar Error
                  </button>
                </div>
                {selected !== null && (
            ...
            {total > 0 && reportOpen && (
              <div className="solid-card animate-fade-in" style={{ padding: "0.875rem", marginBottom: "0.75rem", border: "1px solid rgba(244,63,94,0.25)" }}>
                <label style={{ display: "block", fontSize: "0.75rem", color: "var(--text-tertiary)", marginBottom: "0.375rem", textTransform: "uppercase", fontWeight: 700 }}>
                  ¿Qué problema encontraste?
                </label>
                <textarea className="input textarea" value={reportReason} onChange={(e) => setReportReason(e.target.value)} placeholder="Ej: error de redacción..." style={{ minHeight: 86, marginBottom: "0.625rem" }} />
                <div style={{ display: "flex", justifyContent: "space-between", gap: "0.5rem", flexWrap: "wrap" }}>
                  <span style={{ fontSize: "0.75rem", color: "var(--text-tertiary)" }}>Mínimo 8 caracteres.</span>
                  <div style={{ display: "flex", gap: "0.5rem" }}>
                    <button className="btn btn-secondary btn-sm" onClick={() => setReportOpen(false)} disabled={isReporting}>Cancelar</button>
                    <button className="btn btn-primary btn-sm" onClick={() => reportError(reportReason)} disabled={isReporting}>{isReporting ? "Enviando..." : "Enviar Reporte"}</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {total > 0 && showHint && !showExplanation && !isExamMode && (
              <div className="solid-card animate-fade-in" style={{ padding: "1rem", borderLeft: "3px solid var(--warning-400)", background: "rgba(245,158,11,0.05)" }}>
                <div style={{ display: "flex", gap: "0.5rem", fontSize: "0.875rem", color: "var(--text-secondary)" }}>
                  <span>💡</span>
                  <div><strong>Pista:</strong><MathText>{q.hint}</MathText></div>
                </div>
              </div>
            )}
            ...
            {total > 0 && showExplanation && q?.explanation && !isExamMode && (
              <div className="solid-card animate-fade-in" style={{ padding: "1.25rem", borderLeft: "3px solid var(--accent-400)", background: "rgba(34,211,238,0.05)", marginTop: "0.75rem" }}>
                <p style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--accent-400)", marginBottom: "0.375rem", textTransform: "uppercase" }}>Justificación</p>
      ...
      {!isZenMode && (
        <nav className="quiz-mobile-bottom-nav">
          <Link href="/quiz" className="quiz-mobile-bottom-link"><span>🧭</span><span>Menú</span></Link>
          <Link href="/quiz/repaso" className="quiz-mobile-bottom-link"><span>🧠</span><span>Repaso</span></Link>
          <Link href="/badges" className="quiz-mobile-bottom-link"><span>🏅</span><span>Logros</span></Link>
          <button className="quiz-mobile-bottom-link" onClick={() => setSidebarOpen(!sidebarOpen)}>
            <span>{sidebarOpen ? "✕" : "☰"}</span><span>{sidebarOpen ? "Cerrar" : "Panel"}</span>
          </button>
        </nav>
      )}

      <AdminQuestionEditor {...{ isEditing, setIsEditing, editedQuestion, setEditedQuestion, handleSaveEdit, isSavingEdit }} />
    </div>
  );
}
